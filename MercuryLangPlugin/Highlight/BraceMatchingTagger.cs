...
namespace MercuryLangPlugin.Highlight
{
    internal class BraceMatchingTagger : ITagger<TextMarkerTag>
    {
        ...
        public IEnumerable<ITagSpan<TextMarkerTag>> GetTags(NormalizedSnapshotSpanCollection spans)
        {
            ...
            {
                yield break; //Strange things happens during text modification
            }
            ...
            if (m_braceList.ContainsKey(currentText))   //the key is the open brace
            {
                ...
                {
                    yield return new TagSpan<TextMarkerTag>(new SnapshotSpan(currentChar, 1), PredefinedTextMarkerTags.BraceMatching);
                    yield return new TagSpan<TextMarkerTag>(pairSpan, PredefinedTextMarkerTags.BraceMatching);
                }
            }
            else if (m_braceList.ContainsValue(lastText))    //the value is the close brace, which is the *previous* character 
            {
                ...
                {
                    yield return new TagSpan<TextMarkerTag>(new SnapshotSpan(lastChar, 1), PredefinedTextMarkerTags.BraceMatching);
                    yield return new TagSpan<TextMarkerTag>(pairSpan, PredefinedTextMarkerTags.BraceMatching);
                }
            }
        }
